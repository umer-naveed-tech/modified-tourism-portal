<?php
// hotel_handlers/saja_hotel_makkah.php
//
// Saja Hotel Makkah -- 5 Star
// 3 room categories: Diplomatic Room, Executive Suite, Royal Suite.
// Diplomatic/Executive subtypes: Double/Triple/Quad. Royal Suite
// subtypes: Quad/Quint/6 Pax (alag set, DB se dynamically fetch hota
// hai jaisa Fairmont mein hai). NO weekday/weekend split (rate sheet
// mein sirf date-range based ek price hai). No extra bed, no
// supplement -- International/Far East menu sirf static info text.

require_once __DIR__ . '/base_handler.php';

class SajaHotelMakkahHandler implements HotelHandlerInterface {

    private $hotel_id = SAJA_HOTEL_ID;
    ...
    public function getRooms($hotel_id) {
        ...
        foreach ($rooms as &$room) {
            ...
            $bedStmt = $pdo->prepare("SELECT DISTINCT room_type FROM hotel_seasonal_pricing WHERE hotel_id = ? AND room_type_code = ? ORDER BY FIELD(room_type,'double','triple','quad','quint','sixpax')");
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            🍽️ <strong>International Menu:</strong> Breakfast SAR 45 | Lunch SAR 60 | Dinner SAR 75 (per pax per day)<br>
            🍽️ <strong>Far East Menu:</strong> Full Board SAR 65 (per pax per day)
        </div>
        <?php
        return ob_get_clean();
    }
