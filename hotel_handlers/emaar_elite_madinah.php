<?php
// hotel_handlers/emaar_elite_madinah.php
//
// Emaar Elite Hotel Madinah -- 3 Star
// Double/Triple/Quad, NO weekday/weekend split. Breakfast always included.
// Lunch/Dinner = 105 SR per pax per day -- info text only.

require_once __DIR__ . '/base_handler.php';

class EmaarEliteMadinahHandler implements HotelHandlerInterface {

    private $hotel_id = EMAARELITEMADINAH_HOTEL_ID;
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            🍳 Meal Plan: Breakfast Included<br>🍽️ Lunch or Dinner: SAR 105 per person per day
        </div>
        <?php
        return ob_get_clean();
    }
