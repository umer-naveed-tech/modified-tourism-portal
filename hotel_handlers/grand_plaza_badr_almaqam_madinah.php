<?php
// hotel_handlers/grand_plaza_badr_almaqam_madinah.php
//
// Grand Plaza Badr Al Maqam Hotel Madinah -- 4 Star
// DBL/TPL/QUAD, NO weekday/weekend split. ROOM ONLY basis -- no meal included.
// Breakfast/Lunch/Dinner prices -- info text only.

require_once __DIR__ . '/base_handler.php';

class GrandPlazaBadrAlMaqamMadinahHandler implements HotelHandlerInterface {

    private $hotel_id = GRANDPLAZABADR_HOTEL_ID;
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            🛏️ Meal Plan: Room Only (no meal included)<br>🍳 Breakfast: SAR 45 | 🍽️ Lunch or Dinner: SAR 65
        </div>
        <?php
        return ob_get_clean();
    }
