<?php
// hotel_handlers/dar_al_iman_intercontinental_madinah.php
//
// Dar Al Iman Intercontinental Madinah -- 5 Star
// Single room category (Single/Double), with TWO bed-type variants:
// City View (cv) and Haram View (hv), each with its own weekday/
// weekend price. Extra Bed varies by period (rate sheet) + 25 SAR
// hidden markup. Lunch/Dinner = 180 SR per person per night -- info
// text only. Breakfast always included.

require_once __DIR__ . '/base_handler.php';

class DarAlImanIntercontinentalMadinahHandler implements HotelHandlerInterface {

    private $hotel_id = DARALIMAN_HOTEL_ID;
    ...
    public function getRooms($hotel_id) {
        ...
        foreach ($rooms as &$room) {
            ...
            $bedStmt = $pdo->prepare("SELECT DISTINCT room_type FROM hotel_seasonal_pricing WHERE hotel_id = ? AND room_type_code = ? ORDER BY FIELD(room_type,'cv','hv')");
    ...
    public function calculatePrice($hotel_id, $room_type, $check_in, $check_out, $options = []) {
        ...
        if ($bed_type === '') return ['error' => 'Please select City View or Haram View'];
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div class="extra-bed-option" style="display:block;">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="daralimam_extra_bed" name="extra_bed" value="1" onchange="calculateTotal()">
                <label class="form-check-label" for="daralimam_extra_bed">
                    Add Extra Bed
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            ...
            🍽️ Lunch or Dinner: SAR 180 per person per night
        </div>
        <?php
        return ob_get_clean();
    }
