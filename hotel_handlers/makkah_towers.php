<?php
// hotel_handlers/makkah_towers.php — SIMPLIFIED (Sirf Rooms + Extra Bed)

require_once __DIR__ . '/base_handler.php';

class MakkahTowersHandler implements HotelHandlerInterface {
    ...
    public function calculatePrice($hotel_id, $room_type, $check_in, $check_out, $options = []) {
        ...
        for ($i = 0; $i < $nights; $i++) {
            ...
            if (!$rule) {
                return ['error' => "No pricing available for date: $current_date (Room: $room_type)"];
            }
            
            // 🔴 Room Price (Base + Markup already merged in DB)
            $night_price = $rule['base_price_sar'];
            $total += $night_price;
            
            // 🔴 Extra Bed Price (Base + Markup already merged in DB)
            if ($extra_bed) {
    ...
    // 🔴 SIMPLIFIED UI — Sirf Extra Bed
    public function renderRoomSelection($hotel_id, $rooms) {
        ob_start();
        ?>
        <div class="makkah-towers-options">
            <!-- Extra Bed -->
            <div class="extra-bed-option" style="display:block; margin-top:16px; background:rgba(255,255,255,0.02); padding:16px 20px; border-radius:12px; border:1px solid rgba(255,255,255,0.04);">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="extra_bed_makkah_towers" name="extra_bed" value="1" onchange="calculateTotal()">
                    <label class="form-check-label" for="extra_bed_makkah_towers" style="font-size:13px; color:rgba(255,255,255,0.6);">
                        Add Extra Bed
                        <span style="color:rgba(255,255,255,0.3); font-size:11px; display:block; margin-top:2px;">(Additional charges apply based on season)</span>
                    </label>
                </div>
            </div>
        </div>
