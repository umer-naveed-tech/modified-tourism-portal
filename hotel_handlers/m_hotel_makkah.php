<?php
// hotel_handlers/m_hotel_makkah.php
//
// M Hotel Makkah by Millenium -- 5 Star
// 4 room categories: Double, Triple, Quad, Executive Suite. NO weekday/
// weekend split -- purely date-range based (rate sheet has ek hi price
// per period). No extra bed ("fare mein add nahi hai"). Breakfast /
// Lunch / Dinner sirf static prices, koi selection nahi.

require_once __DIR__ . '/base_handler.php';

class MHotelMakkahHandler implements HotelHandlerInterface {

    private $hotel_id = MHOTEL_HOTEL_ID;
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            🍳 Breakfast: SAR 40 per person<br>
            🍽️ Lunch or Dinner: SAR 60 per person
        </div>
        <?php
        return ob_get_clean();
    }
