<?php
// hotel_handlers/biltmore_madinah.php
//
// The Biltmore Hotel Madinah -- 5 Star
// 11 room categories (Single/Twin/Triple/Family/Junior Suite/
// Executive Suite/Royal Suite/Grand Royal, City View & Haram View
// variants). NO weekday/weekend split (rate sheet gives one price
// per seasonal period). Extra Bed: flat 500 SR (rate sheet, "inc Half
// Board") + 25 SAR hidden markup = 525 SR shown, same every period.
// Lunch/Dinner = 250 SR net per person -- info text only. Meal Plan:
// Bed & Breakfast (B.B), always included.

require_once __DIR__ . '/base_handler.php';

class BiltmoreMadinahHandler implements HotelHandlerInterface {

    private $hotel_id = BILTMORE_HOTEL_ID;
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div class="extra-bed-option" style="display:block;">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="biltmore_extra_bed" name="extra_bed" value="1" onchange="calculateTotal()">
                <label class="form-check-label" for="biltmore_extra_bed">
                    Add Extra Bed
                    <span style="color:rgba(255,255,255,0.3); font-size:11px; display:block; margin-top:2px;">(Includes Half Board -- additional charge per night)</span>
                </label>
            </div>
        </div>
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            ...
            🍽️ Extra Lunch or Dinner: SAR 250 net per person
        </div>
        <?php
        return ob_get_clean();
    }
