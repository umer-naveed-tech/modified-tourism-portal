<?php
// hotel_handlers/manzel_ajyad.php
//
// Ek hi room category, bed-types alag alag, NO weekday/weekend split,
// no extra bed, no supplement. Meal Plan sirf static text.

require_once __DIR__ . '/base_handler.php';

class ManzelAjyadHandler implements HotelHandlerInterface {

    private $hotel_id = MANZELAJYAD_HOTEL_ID;
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            🍤 Meal Plan: Room Only (Breakfast: SAR 40, Half Board: SAR 80, Full Board: SAR 120, Full Board Far East: SAR 60 -- per person per night)
        </div>
        <?php
        return ob_get_clean();
    }
