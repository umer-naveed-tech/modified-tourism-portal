<?php
// hotel_handlers/uthman_ben_affan_madinah.php
//
// Uthman Ben Affan Hotel Madinah -- 4 Star
// Double/Triple/Quad, NO weekday/weekend split. Meal plan NOT specified
// on the rate sheet -- defaulted to Room Only; confirm with Umer if
// this should actually be Breakfast Included.

require_once __DIR__ . '/base_handler.php';

class UthmanBenAffanMadinahHandler implements HotelHandlerInterface {

    private $hotel_id = UTHMANBENAFFAN_HOTEL_ID;
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            🛏️ Meal Plan: Room Only (please confirm meal plan for this hotel)
        </div>
        <?php
        return ob_get_clean();
    }
