<?php
// hotel_handlers/elaf_kinda_makkah.php
//
// Elaf Kinda Hotel Makkah -- 5 Star
// 7 room categories (Double/Twin, Triple, Quadruple, Quadruple H.V,
// Junior Suite H.V, Executive Suite H.V, Family Suite), har ek ka apna
// weekday/weekend price. Bilkul Fairmont Clock Tower jaisa structure,
// lekin har category ka sirf EK "bed type" hai (khud category hi poori
// room hai, alag Double/Triple/Quad selection ki zaroorat nahi).
// No extra bed, no supplement ("add ka feature" -- Umer ke kehne par
// nahi banaya). Meal Plan aur Lunch/Dinner sirf static text.

require_once __DIR__ . '/base_handler.php';

class ElafKindaMakkahHandler implements HotelHandlerInterface {

    private $hotel_id = ELAFKINDA_HOTEL_ID;
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            ...
            🍽️ Lunch: SAR 110 &nbsp;|&nbsp; Dinner: SAR 110
        </div>
        <?php
        return ob_get_clean();
    }
