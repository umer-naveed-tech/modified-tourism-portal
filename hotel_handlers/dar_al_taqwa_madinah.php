<?php
// hotel_handlers/dar_al_taqwa_madinah.php
//
// Dar Al Taqwa Hotel Madinah -- 5 Star
// 4 room categories: DBL (City View), Standard DBL (Haram View),
// Business Suite 1 Bedroom (DBL), Business Junior Suite 1 Bedroom
// (Haram View). Each category has its OWN weekday/weekend price
// (this hotel's rate sheet gives weekday/weekend directly, no
// separate bed-type dimension -- each category IS the bed type).
// Extra Bed: flat 390 SR (rate sheet) + 25 SAR hidden markup = 415 SR
// shown to customer, same every period. Lunch/Dinner = 265 SR per
// person per night -- info text only (not a selectable line item,
// same convention as Sheraton/DoubleTree's static meal notes).
// Breakfast always included.

require_once __DIR__ . '/base_handler.php';

class DarAlTaqwaMadinahHandler implements HotelHandlerInterface {

    private $hotel_id = DARALTAQWA_HOTEL_ID;
    ...
    public function getBookingOptions($hotel_id) {
        return ['extra_bed_available' => true, 'has_weekend_split' => true];
    }
    ...
    public function renderRoomSelection($hotel_id, $rooms) {
        ...
        <div class="extra-bed-option" style="display:block;">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="taqwa_extra_bed" name="extra_bed" value="1" onchange="calculateTotal()">
                <label class="form-check-label" for="taqwa_extra_bed">
                    Add Extra Bed
        ...
        <div style="margin-top:16px; background:rgba(16,185,129,0.04); padding:12px 16px; border-radius:8px; border:1px solid rgba(16,185,129,0.06); font-size:13px; color:#34d399;">
            ...
            🍽️ Lunch or Dinner: SAR 265 per person per night
        </div>
        <?php
        return ob_get_clean();
    }
